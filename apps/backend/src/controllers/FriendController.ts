...
const  updateMyFriendDetails = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const friendData: Prisma.FriendUpdateInput  = {};
       ...
       if (Object.keys(friendData).length === 0) {
            return res.status(400).json({ message: 'No valid friend detail fields provided for update.' });
       }
      ...
      res.status(200).json(updatedFriend);
         ...
         if ((error as any).code === 'P2025') { // Record to update not found
            return res.status(404).json({ message: 'Friend profile not found for this user. Registration might be incomplete.' });
        }
  ...
  const friendController = async(req:Request, res:Response, next:NextFunction) =>{
    const {interests, specialties, gender, minRating, page = 1, limit=20} = req.body;
    try {
